    ...
    await describe(`should process "${fileName}" correctly`, async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          `Expected text output to include "sample" for ${fileName}`
        )
      } catch (error) {
    ...
    await describe(`should throw an error for unsupported file "${fileName}"`, async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          `Expected an Error to be thrown for ${fileName}`
        )

        assert.ok(
          error.message.includes('Error parsing file'),
          `Expected error message to indicate unsupported file type for ${fileName}`
        )
      }
    })
  }
